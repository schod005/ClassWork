...
public class IndividualQStat {
    ...
    public static void filestats(DataOutputStream file){
        ...
        try {
			file.writeBytes(numberOfTellers+"\t"+BankSim.EXPRESS_AVAILABLE+"\t"+maxQueueLength+"\t"+totalAverageQLength/numberOfTellers/totalTime+"\t"+totalAverageServiceTime/numberOfTellers+"\t"+totalAverageWaitTime/totalCustomersServed+"\t"+upsetcustomers+"\t"+totalCustomersServed+"\t"+System.getProperty("line.separator"));
		} catch (IOException e) {
			e.printStackTrace();
		}
