...
public class SharedQStat {
    ...
    public static void filestats(DataOutputStream file){
        ...
        try {
			file.writeBytes(numberOfTellers+"\t"+BankSim.EXPRESS_AVAILABLE+"\t"+maxQLength+"\t"+averageQLength/totalTime+"\t"+totalAverageServiceTime/totalCustomersServed+"\t"+totalAverageSharedQueueWait+"\t"+upsetcustomers+"\t"+totalCustomersServed+"\t"+totalBusyTime/numberOfTellers+"\t"+totalIdleTime/numberOfTellers+"\t"+System.getProperty("line.separator"));
		} catch (IOException e) {
			e.printStackTrace();
		}
