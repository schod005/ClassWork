...
class DHTNode_NodeInterfaceHandler : virtual public DHTNode_NodeInterfaceIf {
  ...
  public:
    ...
    void Write(LogInfo& _return, const WriteFile& fileInfo, const int64_t flags) {
      ...
      int hash = hash_fn(fileInfo.filename);
      hash %= DHT_SIZE;
#ifdef DEBUG
      std::cout << "Hash = " << hash << " ";
#endif
      ...
      {
#ifdef DEBUG
        std::cout << "I am responsible for this file" << std::endl;
#endif
      }
      else
      {
    ...
    void UpdateDHT(const std::vector<UpdateDHTInfo> & NodesList) {
      ...
      {
        m_fingertable[i] = successor(NodesList, (m_ID + (int)pow(2, i - 1)) % DHT_SIZE);
      }
      m_range.second = m_ID;
      ...
      std::cout << "I am responsible for hashes: " << m_range.first << "-"
    ...
    void getInfo(myInfo& _return) {
      ...
      for(std::map<std::string, std::string>::iterator file = m_files.begin(); file != m_files.end(); file++)
        fileList.push_back(file->second);
...
{
  if((m_ID <= id && m_fingertable[1].ID >= id) || ((m_ID > m_fingertable[1].ID) && (m_ID > id && m_fingertable[1].ID >= id)))
    return 1;
  for(unsigned int i = 1; i < m_fingertable.size(); i++)
  {
    ...
    {
      if((m_fingertable[i].ID <= id && (m_fingertable[i + 1].ID < id)) || ((m_fingertable[i].ID > id) && m_fingertable[i].ID >= id))
        return i;
    }
  }
